    ...
    for (simulation in simulations){
      ...
      for (replica in simulation[1:(length(simulation)/2)]){
        ...
        time[i,1] <- system.time({
          boruta <- Boruta(replica$x, replica$y, mtry = 500)
        ...
        time[i,2] <- system.time({
          PerVarImp1 <- CVPVI(replica$x, replica$y, parallel = TRUE, mtry = 500)
        ...
        time[i,3] <- system.time({
          rf <- randomForest(replica$x, replica$y, importance = T, mtry = 500)
        ...
        time[i,4] <- system.time({
          vsurf <- VSURF(replica$x, replica$y, parallel = TRUE, mtry = 500)
...
save(resultats, file = "resultatsMtry500.RData")